...
public class DrawingAreaController implements Initializable {
    ...
    private void addHandlersToResizeRectangle(final Polygon rectangle,
    			...
    			new EventHandler<MouseEvent>() {
					...
					public void handle(final MouseEvent e) {
						if (state == Constants.STATE_RESIZING) {
							currentShapeObject.rectanglePressed(e.getX(), e.getY());
						}
					}
    			});
    	rectangle.addEventHandler(MouseEvent.MOUSE_DRAGGED,
			new EventHandler<MouseEvent>() {
				...
				public void handle(final MouseEvent e) {
					if (state == Constants.STATE_RESIZING) {
						currentShapeObject.resizeDragging(e.getX(), e.getY());
					}
				}
			});
				...
				public void handle(final MouseEvent e) {
					if (state == Constants.STATE_RESIZING) {
						...
						historyFactory.addHistoryNode(currentShapeObject,
							Operations.RESIZE,
							currentShapeObject.getNewScales(),
     ...
     */
	public void loadShapesFromFile(final List<ShapePicaso> shapesLoaded) {
			...
			for (int j = 0; j < sz; j += 2) {
		    	if (!startDraw) { // the first point of the shape
		    		...
		    		if (this.drawnVerticesCounter == 2
		    				* shapesLoaded.get(i).getMinimumNumberOfVericesToDraw()) {
		    			shapesLoaded.get(i).setUpResizeRectangle();
		    			Polygon rectangle = shapesLoaded.get(i).getResizeRectangle();
		    			addHandlersToResizeRectangle(rectangle, shapesLoaded.get(i));
		    			mainArea.getChildren().add(rectangle);
