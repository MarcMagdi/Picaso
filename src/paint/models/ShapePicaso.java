...
/**.
 * Abstract shape class to use it as a contract with all shapes
...
public abstract class ShapePicaso {
     ...
     * @formats fill and stroke colors and stroke width
    ...
    public void rectanglePressed(double x, double y) {
    	if (Math.abs(x - resizeRectangle.getLayoutBounds().getMinX()) < 10) {
    		xLeftSetted = true;
    		xRightSetted = false;
    		prevX = x;
    	} else if (Math.abs(x - resizeRectangle.getLayoutBounds().getMaxX()) < 10) {
    		xRightSetted = true;
    		xLeftSetted = false;
    		prevX = x;
    	} else {
        ...
        if (Math.abs(y - resizeRectangle.getLayoutBounds().getMinY()) < 10) {
    		yBottomSetted = true;
    		yTopSetted = false;
    		prevY = y;
    	} else if (Math.abs(y - resizeRectangle.getLayoutBounds().getMaxY()) < 10) {
    		yTopSetted = true;
    		yBottomSetted = false;
    		prevY = y;
    	} else {
